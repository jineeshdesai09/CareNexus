...
export default async function ConsultationPage({
    ...
    if (opd.Status === "COMPLETED" || opd.Status === "BILLED" || opd.Status === "CLOSED") {
        ...
        return (
            <div className="min-h-full bg-slate-50 dark:bg-zinc-950 relative pb-32">
                <div className="max-w-4xl mx-auto p-4 pt-12 flex flex-col gap-8">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <Link href="/doctor/dashboard" className="text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-100 transition-colors font-medium flex items-center gap-2">
                            ...
                            <h1 className="text-2xl font-bold text-slate-900 dark:text-zinc-50 border-l-2 border-slate-300 dark:border-zinc-700 pl-4">Consultation Summary</h1>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-zinc-900 rounded-3xl shadow-sm border border-slate-100 dark:border-zinc-800 overflow-hidden">
                        <div className="bg-emerald-600 dark:bg-emerald-900/50 p-10 text-center text-white relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 dark:bg-black/20 rounded-full -mr-16 -mt-16 blur-2xl"></div>
                            <div className="absolute bottom-0 left-0 w-24 h-24 bg-black/10 dark:bg-white/5 rounded-full -ml-12 -mb-12 blur-xl"></div>

                            <div className="w-20 h-20 bg-white/20 dark:bg-white/10 rounded-full flex items-center justify-center mx-auto mb-6 backdrop-blur-md">
                            ...
                            <p className="text-emerald-50 dark:text-emerald-100/70 text-lg max-w-md mx-auto">The medical record has been securely saved. You can now print the prescription for the patient.</p>
                        </div>

                        <div className="p-10 space-y-10">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 border-b border-slate-100 dark:border-zinc-800 pb-10">
                                <div className="space-y-1">
                                    <h3 className="text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest">Patient Information</h3>
                                    <p className="text-2xl font-bold text-slate-900 dark:text-zinc-100">{opd.Patient.PatientName}</p>
                                    <p className="text-slate-500 dark:text-zinc-400 font-medium text-lg">{opd.Patient.Age} Years • {opd.Patient.Gender}</p>
                                    <p className="text-slate-400 dark:text-zinc-500 font-medium">ID: {opd.Patient.PatientNo}</p>
                                </div>
                                <div className="md:text-right space-y-1">
                                    <h3 className="text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest">Visit Details</h3>
                                    <p className="text-2xl font-bold text-slate-900 dark:text-zinc-100">{opd.OPDDateTime.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
                                    <p className="text-slate-500 dark:text-zinc-400 font-medium text-lg">Token No: {opd.TokenNo}</p>
                                    <p className="text-slate-400 dark:text-zinc-500 font-medium">OPD ID: #{opd.OPDID}</p>
                                </div>
                            </div>
                ...
                <footer className="fixed bottom-0 left-0 right-0 bg-white dark:bg-zinc-950 border-t border-slate-200 dark:border-zinc-800 py-5 px-8 z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] dark:shadow-none">
                    <div className="max-w-7xl mx-auto flex items-center justify-between">
                        <Link href="/doctor/dashboard" className="px-8 py-3 border border-slate-200 dark:border-zinc-700 rounded-2xl text-base font-bold text-slate-600 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-900 transition-all flex items-center gap-3 shadow-sm active:scale-95">
                        ...
                        <div className="hidden sm:block text-slate-400 dark:text-zinc-500 font-medium text-sm">
    ...
    return (
        <form action={finishConsultation} className="min-h-full pb-32 bg-slate-50 dark:bg-zinc-950 font-inter relative">
            ...
            <header className="bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 sticky top-0 z-40">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        {/* Doctor Info */}
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-700 dark:text-blue-400 font-bold text-xl uppercase ring-2 ring-blue-50 dark:ring-blue-900/20">
                        ...
                        <div className="flex items-center gap-6">
                            <div className="text-right hidden sm:block">
                                <p className="text-xs font-medium text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Appointment</p>
                            ...
                            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-400 border border-amber-200 dark:border-amber-800/50">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <aside className="lg:col-span-3 space-y-6">
                        <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm p-6 border border-slate-100 dark:border-zinc-800">
                            <div className="flex flex-col items-center text-center">
                                <div className="w-24 h-24 rounded-full border-4 border-blue-50 dark:border-zinc-800 mb-4 bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center text-blue-600 dark:text-blue-400 text-3xl font-bold uppercase shadow-inner">
                                    {opd.Patient.PatientName[0]}
                                </div>
                                <h2 className="text-lg font-bold text-slate-900 dark:text-zinc-100">{opd.Patient.PatientName}</h2>
                                <p className="text-sm text-slate-500 dark:text-zinc-400 font-medium">ID: {opd.Patient.PatientNo}</p>
                                <div className="mt-4 flex flex-wrap justify-center gap-2">
                                    <span className="px-2 py-1 bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 text-xs rounded-md border border-slate-200 dark:border-zinc-700">{opd.Patient.Age} / {opd.Patient.Gender}</span>
                                    <span className="px-2 py-1 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 border border-blue-100 dark:border-blue-800/30 text-xs rounded-md">Token: {opd.TokenNo}</span>
                                </div>
                                <div className="mt-4 w-full pt-4 border-t border-slate-50 dark:border-zinc-800/50 text-left">
                                    <p className="text-xs text-slate-400 dark:text-zinc-500 uppercase font-bold mb-2">Chief Complaint</p>
                                    <span className="inline-block w-full px-3 py-2 bg-red-50 dark:bg-red-900/10 text-red-700 dark:text-red-400 text-sm font-semibold rounded-lg border border-red-100 dark:border-red-900/30">
                                        {opd.Description || "No complaint recorded"}
                                    </span>
                                </div>
                                <div className="mt-6 w-full space-y-3 text-left">
                                    <div>
                                        <p className="text-xs text-slate-400 dark:text-zinc-500 uppercase font-bold">Contact</p>
                                        <p className="text-sm text-slate-700 dark:text-zinc-300">{opd.Patient.MobileNo}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-slate-400 dark:text-zinc-500 uppercase font-bold">Blood Group</p>
                                        <p className="text-sm text-slate-700 dark:text-zinc-300">{opd.Patient.BloodGroup || "N/A"}</p>
                                    </div>
                                </div>

                                <div className="mt-6 w-full pt-6 border-t border-slate-100 dark:border-zinc-800 space-y-3">
                        ...
                        {labOrders.length > 0 && (
                            <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm p-6 border border-slate-100 dark:border-zinc-800">
                                <h3 className="text-sm font-bold text-slate-900 dark:text-zinc-100 mb-4 flex items-center gap-2">
                                    <span className="p-1 bg-teal-100 dark:bg-teal-900/30 rounded text-teal-600 dark:text-teal-400">🧪</span>
                                    Recent Lab Orders
                                </h3>
                                <div className="space-y-4">
                                    {labOrders.map(order => (
                                        <div key={order.OrderID} className="text-sm border-l-2 border-teal-500 dark:border-teal-500/50 pl-3 py-1">
                                            <div className="flex justify-between items-start mb-1">
                                                <span className="font-semibold text-slate-800 dark:text-zinc-200">Order #{order.OrderID}</span>
                                                <span className="text-[10px] px-1.5 py-0.5 rounded bg-teal-50 dark:bg-teal-900/20 border border-teal-100 dark:border-teal-800/30 text-teal-700 dark:text-teal-400 font-bold uppercase">
                                                    {order.Status.replace('_', ' ')}
                                                </span>
                                            </div>
                                            <ul className="text-xs text-slate-500 dark:text-zinc-400 list-disc list-inside">
                    ...
                    <div className="lg:col-span-9 space-y-6">
                        ...
                        <section className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-slate-100 dark:border-zinc-800 overflow-hidden">
                            <PrescriptionForm
                                medicines={medicines}
